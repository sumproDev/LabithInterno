...
export function FranchiseForm({ compact = false }: { compact?: boolean }) {
  ...
  const { register, handleSubmit, formState: { errors, isSubmitting }, reset } = useForm<FranchiseInput>({ resolver: zodResolver(franchiseSchema), defaultValues: { enquiryType: "Dealership enquiry", consent: false, website: "" } });
  async function submit(data: FranchiseInput) { setServerError(""); const res = await fetch("/api/enquiry", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...data, formType: "dealership" }) }); if (!res.ok) return setServerError("We could not submit the enquiry. Please try again."); setSent(true); reset(); }
  ...
  return <form className={`premium-form ${compact ? "compact-form" : ""}`} onSubmit={handleSubmit(submit)} noValidate>
    ...
    <div className="form-grid">
      <F label="Full name" e={errors.name?.message}>
        <input
          {...register("name", {
            onChange: (e) => {
      ...
      <F label="Phone number" e={errors.phone?.message}>
        <input
          {...register("phone", {
            onChange: (e) => {
      ...
      <F label="Email address" e={errors.email?.message}><input {...register("email")} type="email" placeholder="you@company.com" /></F>
      <F label="City" e={errors.city?.message}>
        <input
          {...register("city", {
            onChange: (e) => {
              e.target.value = e.target.value.replace(/[0-9]/g, "");
      ...
      <F label="State" e={errors.state?.message}>
        <input
          {...register("state", {
            onChange: (e) => {
              e.target.value = e.target.value.replace(/[0-9]/g, "");
        ...
        />
      </F><F label="Investment range" e={errors.investmentRange?.message}><select {...register("investmentRange")}><option value="">Select a range</option><option>Details available on request</option><option>Would like guidance</option><option>Existing business expansion</option></select></F>
    ...
    <label className="consent"><input type="checkbox" {...register("consent")} /><span>I consent to being contacted about this dealership enquiry and accept the privacy policy.</span></label>{errors.consent && <p className="field-error">{errors.consent.message}</p>}
