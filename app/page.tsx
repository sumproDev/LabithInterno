...
import { getProducts, getTestimonials } from "@/lib/cms";
...
const benefits = [[BadgeCheck, "Premium Product Quality"], [SwatchBook, "Modern Designs"], [Leaf, "Durable Materials"], [Boxes, "Wide Product Range"], [Truck, "Pan-India Supply"], [MapPinned, "Dealership Network"]] as const;
...
export default async function HomePage() {
  ...
  return <>
    ...
    <section className="intro-section section-pad"><Container className="split-layout">
      <Reveal className="intro-collage"><div className="collage-main"><Image src="https://res.cloudinary.com/l5uf8nit/image/upload/v1786532222/labith-interno/kplyomlhafmod6hjb7kb.jpg" alt="Labith Interno fluted panel product" fill sizes="(max-width: 800px) 100vw, 48vw" /></div><div className="collage-detail"><Image src="https://res.cloudinary.com/l5uf8nit/image/upload/v1786531070/labith-interno/h55lwofgl2sdkl5uva5z.webp" alt="UV marble sheet design" fill sizes="260px" /></div><span className="vertical-label">PRODUCT · FINISH · DETAIL</span></Reveal>
      <Reveal className="intro-copy" delay={.1}><p className="eyebrow">ABOUT LABITH INTERNO</p><h2>A Brand Built Around <span>Interior Products</span></h2><p>Labith Interno LLP offers a focused collection of premium interior products for modern residential and commercial spaces, supported by a growing dealership-led market presence.</p><div className="benefit-list">{benefits.map(([Icon, text]) => <div key={text}><Icon /><span>{text}</span></div>)}</div><ButtonLink href="/about-labith-interno">Discover the Brand</ButtonLink></Reveal>
    </Container></section>

    <section className="products-section section-pad"><Container><Reveal><div className="section-top"><SectionHeading eyebrow="THE PRODUCT RANGE" title="Everything Starts with the Right Product" description="Explore UV marble sheets, soffit panels, WPC products, French moldings and decorative interior finishes." /><ButtonLink href="/products" variant="secondary">View All Products</ButtonLink></div></Reveal><div className="featured-grid">{products.map((product, i) => <Reveal key={product.slug} delay={(i % 3) * .04}><ProductCard product={product} /></Reveal>)}</div></Container></section>

    <section className="why-section section-pad"><Image src="https://res.cloudinary.com/l5uf8nit/image/upload/v1786531070/labith-interno/h55lwofgl2sdkl5uva5z.webp" alt="Labith Interno decorative wall product" fill sizes="100vw" className="cover" /><div className="why-overlay" /><Container><Reveal><SectionHeading eyebrow="THE LABITH STANDARD" title="Why Choose Labith Interno Products" description="A design-led product portfolio for dealers and customers." /></Reveal><div className="value-grid">{values.map(([Icon, title, description], i) => <Reveal key={title} delay={i * .05}><article className="value-card"><span className="value-number">0{i + 1}</span><Icon /><h3>{title}</h3><p>{description}</p></article></Reveal>)}</div></Container></section>

    <section className="franchise-home section-pad"><Container className="franchise-layout"><Reveal className="franchise-copy"><p className="eyebrow red-eyebrow">PARTNER WITH THE BRAND</p><h2>Grow with <span>Labith Interno</span></h2><p>Join a dealership model built around premium interior products, strong visual branding and expanding customer demand.</p><ul><li>Premium brand positioning</li><li>High-demand product categories</li><li>Marketing and sales assistance</li><li>Product knowledge support</li><li>Territory-based opportunities</li><li>Ongoing business coordination</li></ul><div className="button-row"><ButtonLink href="/dealership" variant="red">Dealership Opportunity</ButtonLink></div><small>Dealership availability and commercial terms are subject to evaluation and formal agreement.</small></Reveal><Reveal className="franchise-form-card" delay={.1}><FranchiseForm compact /></Reveal></Container></section>

    <section className="testimonials-section section-pad"><Container className="testimonial-layout"><Reveal><SectionHeading eyebrow="CLIENT WORDS" title="What People Say About the Materials" description="Customer and partner feedback managed from the CMS." /></Reveal><Reveal delay={.1}><TestimonialSlider testimonials={testimonials} /></Reveal></Container></section>
