...
import { getCollections, getProducts } from "@/lib/cms";
...
export const metadata: Metadata = pageMetadata(
  ...
  "Browse a masonry gallery of Labith Interno product visuals and available interior material images.",
...
async function getGalleryItems(): Promise<GalleryItem[]> {
  ...
  const used = new Set([...productItems, ...collectionItems].map((item) => item.src));
  ...
  [...productItems, ...collectionItems, ...availableItems]
...
export default async function LabithInternosGalleryPage() {
  const [items, products] = await Promise.all([getGalleryItems(), getProducts()]);
      ...
      <section className="gallery-intro section-pad">
        <Container>
          <div className="gallery-intro-grid">
            ...
            <p>
              Browse the full image collection across UV marble sheets, soffit panels, WPC products, French moldings, louvers, fluted panels, charcoal panels, and PU Stone visuals.
            </p>
          </div>
