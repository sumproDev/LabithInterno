...
export default async function ProductDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <>
    ...
    <section className="spec-section section-pad"><Container><div className="spec-heading"><div><p className="eyebrow">PRODUCT DETAILS</p><h2>Material selection, made simpler.</h2></div><p>Compare finishes, understand ideal applications and review the practical details before choosing {product.title} for your space.</p></div><div className="product-spec-layout"><article className="finish-panel"><span className="spec-number">01</span><SwatchBook /><p className="spec-kicker">FINISH PALETTE</p><h3>Find the right expression.</h3><p>Choose a tone and surface character that works with the light, furniture and material palette of your interior.</p><div className="finish-list">{product.finishes.map(f => <span key={f}>{f}</span>)}</div></article><div className="spec-info-grid"><article className="spec-card"><LayoutGrid /><div><p className="spec-kicker">BEST SUITED FOR</p><h3>Where it works</h3><div className="product-application-list">{product.applications.map(a => <span key={a}>{a}</span>)}</div></div></article><article className="spec-card"><Ruler /><div><p className="spec-kicker">PLANNING</p><h3>Dimensions</h3><p>{product.dimensions}</p></div></article><article className="spec-card"><Wrench /><div><p className="spec-kicker">INSTALLATION</p><h3>Fitting guidance</h3><p>{product.installation}</p></div></article><article className="spec-card"><Sparkles /><div><p className="spec-kicker">AFTERCARE</p><h3>Simple upkeep</h3><p>{product.maintenance}</p></div></article></div></div></Container></section>
